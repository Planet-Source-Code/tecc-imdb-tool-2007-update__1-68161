...
Begin VB.Form frmSearch 
   ...
   Begin MSComctlLib.Toolbar Toolbar1 
      ...
      _Version        =   393216
   End
   Begin MSComctlLib.ListView keep 
      Height          =   4575
      Left            =   0
      TabIndex        =   9
      Top             =   0
...
Dim ll As ListItem
keep.ListItems.Clear

    For i = 1 To lstResults.ListItems.Count
        
        If lstResults.ListItems(i).Checked Then
            Set ll = keep.ListItems.Add(, , lstResults.ListItems(i).Text, , 1)
            ll.SubItems(2) = lstResults.ListItems(i).SubItems(2)
        Else
            'lstResults.ListItems.Item(i).Text = ""
        End If
    Next
    ...
    For i = 1 To keep.ListItems.Count
            Set ll = lstResults.ListItems.Add(, , keep.ListItems(i).Text, , 1)
            ll.SubItems(2) = keep.ListItems(i).SubItems(2)
            ll.Checked = True
    Next
...
Clipboard.Clear
Clipboard.SetText HTML_RES
End Sub

Private Sub Form_Load()
lstResults.ListItems.Add , , " "
End Sub
...
'<li><a href="/title/tt0365467/?fr=c2l0ZT1kZnxteD0yMHxzZz0xfGxtPTIwMHx0dD1vbnxwbj0wfHE9bWF0cml4fGh0bWw9MXxubT1vbg__;fc=12;ft=22;fm=1">Making 'The Matrix'</a> (1999) (TV)</li>
Dim S() As String
Dim S1() As String
Dim S2() As String
Dim S3() As String
...
S = Split(inStrng, SEP_S1, , vbTextCompare)
If UBound(S) > 0 Then
    For i = 1 To UBound(S)
        'cycle through title links
        S1 = Split(S(i), SEP_S2, , vbTextCompare)
        If UBound(S1) > 0 Then
            'valid links
            S2 = Split(S1(0), SEP_S3, , vbTextCompare)
            If UBound(S2) > 0 Then
                'valid title link
                IMDB_CODE = "tt" & S2(0)
                IMDB_TITLE = FixHTMLChars(S2(1))
                'new version splits title again
                
                NV = Split(IMDB_TITLE, SEP_S4, , vbTextCompare)
                If UBound(NV) > 0 Then
                IMDB_TITLE = NV(1)
                End If
                
                S3 = Split(IMDB_TITLE, SEP_S5, , vbTextCompare)
                If UBound(S3) > 0 Then
                    'date exists, parse it
                    S4 = Split(S3(1), SEP_S6)
                    If UBound(S4) > 0 Then
                        'full date
