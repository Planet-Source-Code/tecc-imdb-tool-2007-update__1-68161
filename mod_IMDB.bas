...
Public Type MOVIE_DATA_IMDB
    ...
    Offline As Boolean
    ttID As String
    v4_Tagline As String
    v4_MpaaRating As String
End Type

Public CurrentIMDBdata As MOVIE_DATA_IMDB

Public Const CODES_VER As String = "v0.0.4"
Public CV As String
...
'SUBS THat need UPDATING FOR NEW VARIABLES:
'also note: History file will become invalid
'LoadHistory
'SaveHistory
'LoadCodes
'ParseTitlePage - clear data
'LoadIMDBData
'ShowMovieData
'Actual Object type
'Add_History_Item
...
'new in version 4
Public SPL_TAGLINE_LEFT1 As String  'x29
Public SPL_TAGLINE_RIGHT1 As String 'x30
Public SPL_MPAA_LEFT1 As String  'x31
Public SPL_MPAA_RIGHT1 As String 'x32
...
For i = 0 To UBound(S)
    ...
    If UBound(S1) > 0 Then
        ...
        Select Case LCase(S1(0))
            ...
            'XXXXXXXXXXXXXX NEW FOR VERSION .0.4
            Case "x29"
                SPL_TAGLINE_LEFT1 = S1(1)
            Case "x30"
                SPL_TAGLINE_RIGHT1 = S1(1)
            Case "x31"
                SPL_MPAA_LEFT1 = S1(1)
            Case "x32"
                SPL_MPAA_RIGHT1 = S1(1)
        End Select
    End If
Next
...
Public Sub ParseTitlePage(indatas As String, Optional ttIDCode As String)
Dim SPL() As String
Dim SPL1() As String
Dim SPL2() As String
Dim SPL3() As String
...
Erase SPL1
Erase SPL3
'Open App.Path & "\exp.txt" For Output As #4
'    Print #4, indatas
'Close #4

SPL = Split(indatas, SPL_COUNTRY_LEFT1)
If UBound(SPL) > 0 Then
    'found Country
...
Erase SPL1
Erase SPL3
SPL = Split(indatas, SPL_TAGLINE_LEFT1, , vbTextCompare)

If UBound(SPL) > 0 Then
    'found Tagline
    SPL1 = Split(SPL(1), SPL_TAGLINE_RIGHT1, , vbTextCompare)
    If UBound(SPL1) > 0 Then
        'Tagline here
    
        .v4_Tagline = Trim(SPL1(0))
    End If
End If

'MPAA Rating NEW V 0.0.4
Erase SPL
Erase SPL2
Erase SPL1
Erase SPL3
SPL = Split(indatas, SPL_MPAA_LEFT1, , vbTextCompare)

If UBound(SPL) > 0 Then
    'found mpaa
    SPL1 = Split(SPL(1), SPL_MPAA_RIGHT1, , vbTextCompare)
    If UBound(SPL1) > 0 Then
        'mpaa here
    
        .v4_MpaaRating = Trim(SPL1(0))
    End If
End If

    ' the IMDB Changed the format of thier cover HTML
    ' , they now have a tag called name= in the cover
    ' url, the value usually is poster, so we look for
    ' name="poster" in the html code.
    ' this is as of feb 2007, IMDB will likely continue to
    ' change thier HTML formats so the parse.dat file has prooven
    ' useful.
...
Add_History_Item CurrentIMDBdata
...
With sIMDBDAT
    WNDWS(NWID).pFrm.LoadIMDBData .Country, .CoverURL, .Language, .mDate, .mGenre, .mSypnosys, .mTitle, .Runtime, .userRating, .Offline, .ttID, .v4_Tagline, .v4_MpaaRating
    ...
    If Queue(i).lStatus = 1 Then
        ...
        Select Case Queue(i).sType
            Case 0 'title page
                ...
                ParseTitlePage HTMLBUF, Queue(i).IMDBCODE
                DoEvents
            ...
            Case 1 'image download
                ...
                DoEvents
                With Queue(i)
                    DownloadImage .mPicURL, .sSock, .IMDBCODE
                    DoEvents
                    queueexecute = False
...
Public Sub DownloadImage(sURL As String, lSock As Winsock, Optional sTTID As String)
Dim MHOST As String
Dim Mpath As String
Dim MFile As String
